...
const SplashCursor: React.FC<SplashCursorProps> = ({
  ...
  useEffect(() => {
    try {
      ...
      if (!context || !context.ext || !context.ext.formatRGBA || !context.ext.formatRG || !context.ext.formatR) {
        console.warn("WebGL fluid simulation formats not supported on this browser/GPU.");
        return;
      }
    ...
    function updatePointerMoveData(pointer: PointerPrototype, posX: number, posY: number, color: ColorRGB) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas!.width;
      pointer.texcoordY = 1.0 - posY / canvas!.height;
    ...
    updateFrame();
      ...
      console.warn("SplashCursor WebGL context failed to initialize gracefully:", e);
    }
  }, [
    SIM_RESOLUTION,
    DYE_RESOLUTION,
